...
class DirewolfTest < Minitest::Test
  ...
  def setup
    ...
    @wolf_2 = Direwolf.new('Shaggydog', "Winterfell", "Smol Pupper")
    ...
    @john_stark_2 = Stark.new('John', "King's Landing")
  ...
  def test_can_have_another_home_or_size
    assert_equal "Shaggydog", @wolf_2.name
    assert_equal 'Winterfell', @wolf_2.home
    assert_equal "Smol Pupper", @wolf_2.size
  end

  def test_starks_are_in_winterfell_by_default
    assert_equal 'Winterfell', @summer_wolf.home
    assert_equal 'Winterfell', @bran_stark.location
  end

  def test_starts_off_with_no_Starks_to_protect
    assert_equal [], @wolf.starks_to_protect
    assert_equal 'Arya', @arya_stark.name
    assert_equal 'Winterfell', @arya_stark.location
    assert_equal 'Beyond the Wall', @wolf.home
  end

  def test_protects_stark_kid
  ...
  def test_hunts_white_walker
    assert @wolf.hunts_white_walkers?
  end

  def test_hunts_white_walkers_but_not_if_protecting_stark
    @lady_wolf.protects(@sansa_stark)
    refute @lady_wolf.hunts_white_walkers?
  end
